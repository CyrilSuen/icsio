...
export const ZH_CN_MESSAGES = {
    ...
    "auth.between_the_paper_and_ink_the_pen_comes_to_life_an_icsio_is_used_to_p": "纸墨之间，落笔生辉，安放所有想法。",
    ...
    "markdown.info": "信息",
    "markdown.icsio_code_highlighting_failed_showing_plain_text": "[icsio] 代码高亮加载失败，将显示纯文本：",
    "markdown.icsio_diagram_rendering_failed_to_load": "[icsio] 图表渲染加载失败：",
    "markdown.icsio_math_rendering_failed_to_load": "[icsio] 公式渲染加载失败：",
    ...
    "pwa.install": "安装",
    "pwa.install_description": "在独立窗口中打开 icsio，并让应用外壳保持离线可用。",
    "pwa.install_icsio": "安装 icsio",
    "pwa.installed": "已安装",
    "pwa.offline_ready": "完整离线资源已准备好",
    "pwa.offline_ready_description": "icsio 的所有功能现在都能在这台设备上断网打开。",
    "pwa.complete_offline_access": "完整离线能力",
    "pwa.complete_offline_preparing_description": "icsio 会保持流畅，并在后台安静地补齐其余功能。",
    ...
    "settings.update_dialog_description": "发现 icsio {version}。打开官方仓库并手动同步你的 Fork。",
    "settings.update_dialog_title": "发现新版本",
    "settings.update_manual_fork_hint": "icsio 不会自动修改或部署你的 Fork。请在官方仓库确认变更后手动同步。",
    ...
    "settings.backup_manifest_not_found": "没有找到 icsio Markdown 备份清单，请选择解压后的完整备份目录（应包含 manifest.json、COMPLETE、notes 等）",
    ...
    "settings.includes_every_note_folder_tag_and_attachment_for_a_complete_restore_plu": "下载与自动备份相同的完整 ZIP；超大备份可解压后选择目录，icsio 会分批校验并恢复",
    "settings.indent_width": "缩进宽度",
    "settings.icsio_import_reminder": "[icsio] 导入提醒:",
    ...
    "settings.mcp_demo_desc": "这里按已配置的 icsio 服务器完整展示全部 MCP 选项。端点、凭据、客户端和索引统计均为示例；Demo 中所有 MCP 操作都已禁用。",
    ...
    "settings.region": "区域",
    "settings.restore_backup_folder": "恢复 icsio 备份文件夹",
    "settings.restore_backup_folder_description": "选择新 ZIP 解压后的目录，或旧版同时包含 attachments 和 snapshots 的备份根目录；icsio 会验证 COMPLETE 并分批恢复",
    ...
    "settings.supports_md_txt_zip_and_icsio_json_exports_for_matching_ids_the_newer": "支持 .md / .txt / .zip / icsio 导出的 .json。同 ID 的笔记会保留较新的一份",
    "settings.sync": "同步",
    ...
    "settings.totp_recovery_file_title": "icsio 二次验证恢复码",
    ...
    "public.empty_filter": "该分组下暂无笔记",
    "public.back_home": "返回首页",
    "public.search_placeholder": "搜索标题、标签…",
    "public.hero_subtitle": "记录、分享、沉淀——你的自托管 Markdown 笔记本。",
    "public.home": "首页",
    "public.categories": "分类",
    "public.tags": "标签",
    "public.about": "关于",
    "share.listed_public": "公开上榜",
    "share.listed_public_hint": "无需登录即可在公开首页看到这篇笔记",
...
> icsio 会隔离账号并校验权限，但已授权客户端实际读取到的内容，之后仍由该客户端处理。
...
~~~~md-example title="图片"
![icsio 项目 Logo](/icsio-logo.svg "icsio 项目 Logo")
~~~~

~~~~md-example title="双链"
[[我的第一篇笔记|打开或创建笔记]] · [[欢迎使用 icsio#Markdown 速查|跳到本节]]
~~~~
...
按 \`Ctrl + ,\` 可以修改界面语言、主题、排版、编辑器、同步和备份。读到这里，你已经知道使用 icsio 所需的一切；可以保留这篇速查，也可以放心删除它，开始写自己的内容。 #入门
`,
