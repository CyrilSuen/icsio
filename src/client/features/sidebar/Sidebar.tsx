...
function ViewItem({ icon, label, view, count, active, onSelect, }: {
    ...
    return (<button type="button" aria-current={active ? 'page' : undefined} onClick={() => onSelect(view)} onDragOver={(e) => {
            if (!acceptsDrop || !e.dataTransfer.types.includes('application/x-icsio-note'))
                return;
            e.preventDefault();
            setDropping(true);
        }} onDragLeave={(e) => {
            ...
            setDropping(false);
            const id = e.dataTransfer.getData('application/x-icsio-note');
            if (!id)
                return;
            e.preventDefault();
            if (view === 'unfiled')
...
function FolderSection() {
    ...
    return (<>
      <section className={cn('mt-4 rounded-[var(--r-md)]', rootDropping && 'ring-1 ring-[var(--accent)]')} onDragOver={(event) => {
            if (!event.dataTransfer.types.includes('application/x-icsio-folder'))
                return;
            ...
            const folderId = event.dataTransfer.getData('application/x-icsio-folder');
            if (!folderId)
                return;
            event.preventDefault();
            setRootDropping(false);
...
function FolderRow({ node, siblings, index, parentNode, parentSiblings, onCreateChild, onMove, onChooseParent, onEditAppearance, createdFolderId, renamingId, onStartRename, onFinishRename, }: {
    ...
    return (<div role="treeitem" aria-level={node.depth + 1} aria-expanded={hasChildren ? expanded : undefined} className={cn(justCreated && 'anim-tree-item-enter')} data-new-folder={justCreated || undefined}>
      <div ref={buttonRef} onContextMenu={(event) => {
            ...
            if (!e.dataTransfer.types.includes('application/x-icsio-note') &&
                !e.dataTransfer.types.includes('application/x-icsio-folder'))
                return;
            e.preventDefault();
            e.stopPropagation();
            ...
            e.stopPropagation();
            setDropState('none');
            const noteId = e.dataTransfer.getData('application/x-icsio-note');
            if (noteId) {
                void patchNote(noteId, { folderId: node.id });
                return;
            }
            const folderId = e.dataTransfer.getData('application/x-icsio-folder');
            ...
            e.dataTransfer.setData('application/x-icsio-folder', node.id);
