...
async function saveDownload(format: 'json' | 'zip'): Promise<void> {
  if (format === 'zip') {
    ...
    if (picker) {
      ...
      try {
        handle = await picker.call(window, {
          suggestedName: `icsio-backup-${new Date().toISOString().replace(/[-:TZ]/g, '').slice(0, 15)}.zip`,
      ...
      const { response } = await fetchDownload('/api/export?format=zip', 'icsio-backup.zip')
    ...
    const { response, filename } = await fetchDownload('/api/export?format=zip', 'icsio-backup.zip')
    await saveResponseDownload(response, filename)
    return
  }

  const { response, filename } = await fetchDownload('/api/export?format=json', 'icsio-export.json')
  await saveResponseDownload(response, filename)
}
