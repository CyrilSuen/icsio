...
export const localDb = {
  ...
  async withOutboxReplayLock(owner: string, task: () => Promise<void>): Promise<boolean> {
    const lockName = activeUserId ? `icsio-outbox-replay:${activeUserId}` : 'icsio-outbox-replay'
...
export function publishBroadcast(payload: BroadcastPayload): void {
  ...
  try {
    broadcastPublisher ??= new BroadcastChannel('icsio')
...
export function createBroadcast(
  ...
  const channel = new BroadcastChannel('icsio')
