...
async function highlightCodeBlocks(root: HTMLElement): Promise<void> {
    await Promise.all([...root.querySelectorAll<HTMLElement>('.code-block')].map(async (block) => {
        ...
        try {
            ...
            code.textContent = source;
            console.warn(t("markdown.icsio_code_highlighting_failed_showing_plain_text"), err);
        }
    ...
    if (!katexPromise) {
        ...
        void loading.catch((err) => {
            if (katexPromise === loading)
                katexPromise = null;
            console.warn(t("markdown.icsio_math_rendering_failed_to_load"), err);
        });
    }
    try {
    ...
    if (!mermaidPromise) {
        ...
        void loading.catch((err) => {
            if (mermaidPromise === loading)
                mermaidPromise = null;
            console.warn(t("markdown.icsio_diagram_rendering_failed_to_load"), err);
        });
    }
