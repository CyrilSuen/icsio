    ...
    for (const value of files) {
      try {
        if (value.name.toLowerCase().endsWith('.zip')) {
          ...
          const bundleEntry = entries.find((entry) => entry.path.endsWith('icsio-export.json'))
          if (!bundleEntry) throw new Error('The ZIP does not contain an icsio export')
          await importBundle(
            state,
...
function demoMcpSettings(now = Date.now()): McpSettingsInfo {
  return {
    ...
    endpoint: 'https://your-icsio.example/mcp',
    oauth: true,
    preferences: {
      writeEnabled: true,
      trashEnabled: false,
...
async function exportResponse(state: DemoState, format: 'json' | 'zip'): Promise<Response> {
  ...
  if (format === 'json') {
    return new Response(JSON.stringify(baseBundle, null, 2), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="icsio-demo.json"',
      },
    })
  }
  ...
  const entries = [
    { path: 'icsio-export.json', data: encoder.encode(JSON.stringify(bundle, null, 2)) },
    ...noteEntries,
    ...attachmentEntries,
  ]
  ...
  return new Response(data as BodyInit, {
    headers: {
      ...
      'Content-Disposition': 'attachment; filename="icsio-demo.zip"',
    },
  })
}
