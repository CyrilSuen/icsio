...
import { createicsioMcpServer, type McpAuthProps } from './server'
import { isMcpEnabled, MCP_SUPPORTED_SCOPES } from './settings'

const app = createApp()

export class icsioMcpApi extends WorkerEntrypoint<Env, McpAuthProps> {
  async fetch(request: Request): Promise<Response> {
    const database = await initializeDatabase(this.env)
    if (!await isMcpEnabled(this.env.DB)) {
      return Response.json({ error: 'MCP is disabled in icsio settings' }, { status: 403 })
    }
...
export function providerForScheduled(env: Env): OAuthProvider<Env> {
  const origin = configuredOrigin(env.PUBLIC_URL) ?? 'https://icsio.invalid'
  return providerForOrigin(origin, env)
}
...
function providerForOrigin(origin: string, env: Env): OAuthProvider<Env> {
  ...
  return new OAuthProvider<Env>({
    ...
    resourceMetadata: {
      ...
      resource_name: 'icsio private knowledge base',
    },
    ...
    onError(error) {
      if (error.internal) {
        console.warn('[icsio] OAuth error:', error.internal.category, error.internal.reason)
      }
    },
  })
}
