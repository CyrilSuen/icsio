...
export interface icsioMcpServerOptions {
  env: Env
  auth: McpAuthProps
  origin: string
  ftsEnabled: boolean
...
const noteId = z.string().refine(isValidId, 'Invalid icsio note id')
const expectedRev = z.number().int().positive().describe('Current note rev returned by read_note or fetch')

export function createicsioMcpServer(options: icsioMcpServerOptions): McpServer {
  const server = new McpServer(
    { name: 'icsio Knowledge Base', version: '1.0.0' },
  ...
  server.registerTool(
    ...
    {
      title: 'Search icsio',
      description: 'Search private icsio notes by keyword and meaning. Returns citation-ready note ids, titles, and absolute URLs. Use before fetch.',
...
async function writeTool(
  ...
  return safeTool(async () => {
    ...
    if (scope === MCP_SCOPES.write && !preferences.writeEnabled) {
      throw ApiError.forbidden('MCP writes are disabled in icsio settings')
    }
    if (scope === MCP_SCOPES.trash && !preferences.trashEnabled) {
      throw ApiError.forbidden('MCP trash access is disabled in icsio settings')
    }
...
function toolError(error: unknown): Record<string, unknown> {
  ...
  console.error('[icsio] MCP tool failed:', error)
  return { error: { code: 'internal', message: 'icsio could not complete the tool call' } }
}
