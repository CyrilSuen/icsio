...
export async function createMcpFolder(
  ...
  if (!isValidId(id)) throw ApiError.badRequest('folder_id must be a valid icsio id')
...
export async function createMcpTag(
  ...
  if (!isValidId(id)) throw ApiError.badRequest('tag_id must be a valid icsio id')
  return runIdempotent({
    db: context.env.DB,
    userId: context.userId,
    operationId: input.operationId,
...
export async function uploadMcpAttachment(
  ...
  if (!isValidId(id)) throw ApiError.badRequest('attachment_id must be a valid icsio id')
