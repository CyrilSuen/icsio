...
export async function runBackup(env: Env, userId: string, options: RunOptions): Promise<BackupRun> {
  ...
  const heartbeat = setInterval(() => {
    void release.renew().then((ok) => {
      if (!ok) console.error(`[icsio] Backup lease was lost for ${userId}`)
    }).catch((error) => {
      console.warn('[icsio] Backup lease renewal failed:', error instanceof Error ? error.message : error)
    })
...
async function persistRunSafely(
  ...
  try {
    ...
    console.error(`[icsio] Backup completed, but writing the run record failed (${run.id}):`, error)
  }
}
...
async function recordOutcomeSafely(
  ...
  try {
    ...
    console.error(`[icsio] Backup completed, but writing target state failed (${run.id}):`, error)
  }
}
