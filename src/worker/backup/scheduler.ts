...
export async function runScheduledBackups(env: Env): Promise<void> {
  try {
    ...
    console.error('[icsio] Scheduled task: database is not ready', err)
    return
  }
  ...
  while (true) {
    ...
    await forEachConcurrent(users, 2, async (user) => {
      try {
        ...
        console.log(
          `[icsio] Scheduled backup ${user.id}: ${run.status}, ${run.results.length} targets, ${run.bytes} bytes`,
        )
      } catch (err) {
        console.error(`[icsio] User ${user.id} scheduled backup failed:`, err)
      }
    })
  ...
  try {
    ...
    console.warn('[icsio] Failed to trim the change log:', error)
  }
}
