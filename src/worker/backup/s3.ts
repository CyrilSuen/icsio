...
async function s3ArchiveMatches(
  ...
  const storedStamp = response.headers.get('X-Amz-Meta-icsio-Snapshot')
  await response.body?.cancel().catch(() => {})
...
function archiveHeaders(stamp: string): Record<string, string> {
  return {
    ...
    'X-Amz-Meta-icsio-Backup': 'markdown-v3',
    'X-Amz-Meta-icsio-Snapshot': stamp,
...
async function abortMultipart(
  ...
  try {
    ...
    if (!response.ok && response.status !== 404) {
      console.warn(`[icsio] S3 multipart cleanup failed: HTTP ${response.status} (${key})`)
    }
  } catch (error) {
    console.warn('[icsio] S3 multipart cleanup failed:', friendlyError(error))
  }
}
...
export async function s3Test(
  ...
  try {
    ...
    const key = joinKey(
      ...
      `.icsio-check-${crypto.randomUUID()}`,
    )
    const url = objectUrl(config, key)
    const payload = new TextEncoder().encode(`icsio ${new Date().toISOString()}`)
    let written = false
    ...
    try {
      ...
      if (written) {
        ...
        if (cleanupError) {
          ...
          console.warn('[icsio] S3 test object cleanup failed:', cleanupError.message)
        }
      }
    }
  } catch (err) {
