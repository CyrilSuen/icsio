...
authRoutes.post('/register', async (c) => {
  ...
  await seedWorkspace(c.env, id, locale).catch((err) => {
    console.warn('[icsio] Failed to initialize sample content; the account remains usable:', err)
  })
...
authRoutes.put('/profile', requireAuth, async (c) => {
  ...
  if (previousAvatar) {
    await drainAttachmentCleanup(c.env, current.id).catch((error) => {
      console.warn('[icsio] Replaced avatar cleanup will retry later:', error)
    })
  }
