...
const EXPORT_FILE = 'icsio-export.json'
const EXPORT_FORMAT = 'icsio-export'
...
transferRoutes.get('/export', async (c) => {
  ...
  try {
    ...
    if (format === 'json') {
      ...
      return new Response(bundle as BodyInit, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="icsio-export-${formatStamp(new Date())}.json"`,
    ...
    void archive.stream.pipeTo(fixed.writable).catch((error) => {
      console.error('[icsio] Streaming ZIP export failed:', error)
    })
    ...
    try {
      if (name.endsWith('.zip')) {
        ...
        strictBackup = controls.some((entry) =>
          isMarkdownBackupControlPath(entry.path) || isicsioBackupManifest(entry),
        )
        const backup = await selectCompleteZipBackup(controls)
        if (strictBackup && !backup) {
          throw new Error('The icsio backup ZIP is missing a valid manifest or COMPLETE marker')
        }
        if (backup) {
        ...
        if (bundles.length > 1) {
          throw new Error('The ZIP contains multiple icsio-export.json files and is ambiguous')
        }
        if (bundleEntry) {
...
function parsePostedBackupManifest(value: string | File | null): MarkdownBackupManifest | null {
  ...
  if (!manifest) throw ApiError.badRequest('This is not a valid icsio Markdown backup manifest')
  return manifest
}
...
async function importBundle(
  ...
  if (
    ...
    throw new Error('This is not a valid icsio export')
  }
...
async function prepareBundleAttachments(
  ...
  try {
    ...
    await rollbackPersistedAttachments(
      ...
      console.warn('[icsio] Attachment import rollback was incomplete; the cleanup queue will continue:', rollbackError)
    })
    throw error
  }
...
function isicsioBackupManifest(entry: UnzippedEntry): boolean {
...
async function selectCompleteZipBackup(entries: readonly UnzippedEntry[]): Promise<{
  ...
  for (const entry of entries) {
    ...
    const declaresicsio = isRecord(raw) && raw.format === MARKDOWN_BACKUP_FORMAT
    if (legacyPath || declaresicsio) manifestSeen = true
    const manifest = parseMarkdownBackupManifest(raw)
    if (!manifest) {
      if (siblingComplete && declaresicsio) {
  ...
  if (manifestSeen) throw new Error('The ZIP contains an incomplete icsio backup without a valid COMPLETE marker')
  return null
}
