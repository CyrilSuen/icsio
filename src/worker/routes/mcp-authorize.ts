...
const CSRF_COOKIE = 'icsio_mcp_csrf'
const LOCALE_COOKIE = 'icsio_mcp_locale'
const LOCALE_QUERY = 'icsio_lang'
    ...
    <div class="panel-body">
      ...
      <form id="login" class="login-form">
        <section class="form-section" aria-labelledby="account-title">
          <h2 id="account-title">${copy.icsioAccount}</h2>
      ...
      document.getElementById('login').addEventListener('submit', async (event) => {
        ...
        try {
          const response = await fetch('/api/auth/login', {
            ...
            headers: {'Content-Type':'application/json','X-icsio-Client':'1'},
...
function errorPage(message: string, locale: AppLocale, currentUrl: string): string {
  ...
  return page(`
    <div class="panel-body">
      ...
      <div class="actions"><a class="primary link" href="/">${copy.openicsio}</a></div>
...
function authorizationCopy(locale: AppLocale) {
  if (locale === 'zh-CN') {
    return {
      ...
      passwordOnly: '密码只会发送到当前 icsio 部署。',
      icsioAccount: 'icsio 账户',
      username: '用户名',
      password: '密码',
      signInContinue: '登录并继续',
      needAccount: '还没有账户？请先在另一个标签页打开 icsio。',
      signInFailed: '登录失败',
      authorizationFailed: '授权失败',
      openicsio: '打开 icsio',
      mcpDisabled: 'MCP 已在 icsio 设置中停用。',
      unknownClient: '未知的 OAuth 客户端。',
      sessionExpired: 'icsio 会话已过期，请登录后重试。',
    }
  }
  return {
    header: 'MCP',
    ...
    passwordOnly: 'Your password is sent only to this icsio deployment.',
    icsioAccount: 'icsio account',
    username: 'Username',
    password: 'Password',
    signInContinue: 'Sign in and continue',
    needAccount: 'Need an account? Open icsio in another tab first.',
    signInFailed: 'Sign-in failed',
    authorizationFailed: 'Authorization failed',
    openicsio: 'Open icsio',
    mcpDisabled: 'MCP is disabled in icsio settings.',
    unknownClient: 'Unknown OAuth client.',
    sessionExpired: 'Your icsio session expired. Sign in and try again.',
  }
}
...
function page(content: string, locale: AppLocale, currentUrl: string): string {
  ...
  return `<!doctype html><html lang="${locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>icsio · ${escapeHtml(copy.authorization)}</title><style>
