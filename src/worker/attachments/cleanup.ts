...
export async function drainAttachmentCleanup(
  ...
  for (const storage of ['r2', 'kv'] as const) {
    ...
    try {
      ...
      console.warn('[icsio] Attachment object cleanup will retry later:', error)
    }
  }
...
export async function runAttachmentCleanup(env: Env): Promise<void> {
  try {
    ...
    console.warn('[icsio] Attachment object cleanup will retry during the next scheduled run:', error)
  }
}
