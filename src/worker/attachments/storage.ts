    ...
    if (storedObject) {
      try {
        ...
        await env.DB.prepare(
          ...
          .run()
          .catch(() => {})
        console.warn('[icsio] Attachment cleanup after a write rollback will retry later:', cleanupError)
      }
    }
    throw error
  }
...
export async function rollbackPersistedAttachments(
  ...
  await drainAttachmentCleanup(env).catch((error) => {
    console.warn('[icsio] Attachment cleanup after an import rollback will retry later:', error)
  })
}
