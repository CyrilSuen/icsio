  ...
  return {
    ...
    async req(method, path, body, headers = {}) {
      ...
      for (const c of res.headers.getSetCookie?.() ?? []) {
        const kv = c.split(';')[0]
        if (kv.startsWith('icsio_session=')) jar.cookie = kv.split('=')[1] ? kv : ''
      }
...
async function importJson(client, bundle, filename = 'icsio-export.json') {
...
console.log('[setup]')
{
  ...
  check('site name is icsio', s.data?.site?.name === 'icsio', JSON.stringify(s.data?.site))
...
console.log('[register owner]')
{
  ...
  const englishWelcome = list.find((note) => /^Welcome to icsio/u.test(note.title ?? ''))
  const chineseWelcome = list.find(
    (note) => note.id !== englishWelcome?.id && /icsio/u.test(note.title ?? ''),
  )
  check(
...
if (noteId) {
  const streamedOversize = await fetch(BASE + '/api/files', {
    method: 'POST',
    headers: {
      'Content-Type': 'multipart/form-data; boundary=icsio-e2e',
      'X-icsio-Client': '1',
  ...
  const spoofedUpload = await fetch(BASE + '/api/files', {
    method: 'POST',
    headers: { 'X-icsio-Client': '1', Cookie: owner.jar.cookie },
    body: spoofedForm,
  })
  ...
  const form = new FormData()
  form.set('file', new Blob(['icsio protected attachment'], { type: 'text/plain' }), 'e2e.txt')
  ...
  if (attachment?.id && slug) {
    ...
    const shareCookieName = `icsio_share_${slug}`
    ...
    check(
      ...
      withCookie.status === 200 &&
        ...
        (await withCookie.text()) === 'icsio protected attachment',
...
console.log('[registration toggle]')
{
  ...
  const memberEnglishWelcome = memberList.find((note) => /^Welcome to icsio/u.test(note.title ?? ''))
  const memberChineseWelcome = memberList.find(
    (note) => note.id !== memberEnglishWelcome?.id && /icsio/u.test(note.title ?? ''),
  )
  check(
  ...
  const restoredResponse = await fetch(BASE + '/api/import', {
    method: 'POST',
    headers: { 'X-icsio-Client': '1', Cookie: member.jar.cookie },
    body: restoreForm,
  })
