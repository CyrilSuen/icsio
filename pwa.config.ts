...
export function icsioPwa(): Plugin {
  return {
    name: 'icsio:pwa',
    apply: 'build',
...
function serviceWorkerSource(buildId: string, coreUrls: string[], allUrls: string[]): string {
	...
	const SHELL_CACHE = ${JSON.stringify(`icsio-shell-${buildId}`)}
	const ASSET_CACHE = 'icsio-assets-v1'
	...
	const CACHE_META_URL = '/.icsio-cache-meta'
	const MANIFEST_META_PREFIX = '/.icsio-offline-manifest/'
...
self.addEventListener('install', (event) => {
  event.waitUntil((async () => {
    const previousShells = (await caches.keys()).filter((key) =>
      key.startsWith('icsio-shell-') && key !== SHELL_CACHE)
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(keys
      .filter((key) => key.startsWith('icsio-shell-') && key !== SHELL_CACHE)
